...
**Acceptance Criteria:**
- [ ] `llm-pretrain doctor` reports OS/WSL, Python, PyTorch, CUDA, BF16, Muon, SDPA, compile,
      memory, disk, and sequential I/O checks as structured output.
- [ ] Strict mode returns non-zero when required training checks fail.
- [ ] Apache-2.0, ignore rules, source package, CLI entry point, and synthetic fixture policy exist.
- [ ] `uv run pyright`, `uv run ruff check .`, and focused tests pass.
...
**Acceptance Criteria:**
- [ ] Config types load, validate, and serialize YAML without accepting unknown fields.
- [ ] Default architecture has 99–102M trainable parameters and tied embedding/output weights.
- [ ] CPU tests prove shape, causality, RoPE behavior, loss computation, and small-config backward.
- [ ] `uv run pyright`, `uv run ruff check .`, and focused tests pass.
...
**Acceptance Criteria:**
- [ ] SentencePiece BPE uses 24,576 pieces, byte fallback, and fixed IDs for seven special tokens.
- [ ] Training reads only train-split documents and evaluation reports fertility, bytes/token,
      fallback rate, and normalized round-trip results.
- [ ] Tests use local tiny fixtures and never download data.
- [ ] `uv run pyright`, `uv run ruff check .`, and focused tests pass.
...
**Acceptance Criteria:**
- [ ] Source definitions encode 80% Ultra-FineWeb zh, 10% Chinese Wikimedia, 10% FineWeb en,
      with revision, license, and requested token counts.
- [ ] Normalized SHA-256 deduplication precedes deterministic document-level train/val splitting.
- [ ] EOS packing emits 1024-token uint32 shards, manifests, and resumable batch cursors.
- [ ] Tests prove split disjointness, packing, shard reading, and cursor continuation.
- [ ] `uv run pyright`, `uv run ruff check .`, and focused tests pass.
...
**Acceptance Criteria:**
- [ ] Optimizer partitioning is exhaustive/disjoint and excludes embeddings/head/norm from Muon.
- [ ] Muon and AdamW defaults, warmup+cosine scheduling, BF16 accumulation, finite checks,
      clipping, and global-token accounting match the Decisions.
- [ ] Missing `torch.optim.Muon` produces an actionable version error.
- [ ] CPU focused tests cover partitioning, schedules, accumulation, and failure paths.
- [ ] `uv run pyright`, `uv run ruff check .`, and focused tests pass.
...
**Acceptance Criteria:**
- [ ] Atomic checkpoints include model, all optimizers/schedulers, RunState, sampler cursor, and RNG.
- [ ] Retention keeps latest three plus best without deleting unrelated paths.
- [ ] JSONL and optional TensorBoard record loss, LR, norms, throughput, memory, temperature,
      evaluation, and checkpoint events.
- [ ] CPU resume equivalence and retention tests pass.
- [ ] `uv run pyright`, `uv run ruff check .`, and focused tests pass.
...
**Acceptance Criteria:**
- [ ] A/B runs share initialization, data order, 100M-token budget, global batch, and schedule.
- [ ] Gate requires finite metrics, Muon mean validation loss ≤ AdamW + 0.05, and throughput ≥80%.
- [ ] A failed first gate permits only 0.01/0.04 Muon LR calibration at 50M tokens before stopping.
- [ ] Formal token target is `min(2B, measured_e2e_tokens_per_second * 120h * 0.9)` aligned to batch.
- [ ] Tests cover pass, calibration, stop, and budget calculation.
- [ ] `uv run pyright`, `uv run ruff check .`, and focused tests pass.
...
**Acceptance Criteria:**
- [ ] COIG selection deterministically yields up to 50K train and 1K validation examples.
- [ ] Chat formatting masks user/system tokens and trains only assistant response tokens.
- [ ] SFT defaults to full-parameter AdamW, 5e-5, two epochs, max length 1024.
- [ ] Generation supports seed, EOS, temperature, top-k, prompt mode, and interactive mode.
- [ ] CPU tests cover selection, masking, evaluation, and deterministic generation.
- [ ] `uv run pyright`, `uv run ruff check .`, and focused tests pass.
...
**Acceptance Criteria:**
- [ ] Every planned CLI command has help text, config validation, and actionable errors.
- [ ] README documents WSL setup, data licensing caveats, dry run, A/B, manual long run, resume,
      TensorBoard, SFT, generation, and expected limitations.
- [ ] Tracked-file audit fails on forbidden directories/extensions, secrets, or oversized files.
- [ ] Ralph dry-run and the aggregate validation commands pass.
- [ ] `uv run pyright`, `uv run ruff check .`, and all tests pass.
